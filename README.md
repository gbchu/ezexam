...
+ 为 `solution-block` 方法添加参数 `paper` ，该参数可以设置解析页面显示方式，比如经常试卷页需要 `a3` 显示，解析页 `a4 `显示，此时该参数就派上用场了
...
### 0 . 2 . 4
+ 优化 `choices` 方法；新增参数 `label-position` ； 该参数可在选项为图表时，修改标签的位置。默认在左侧；将参数 `body-indent` 名修改为 `sapcing`
